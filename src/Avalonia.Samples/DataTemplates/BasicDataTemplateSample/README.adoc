...
You should have basic knowledge about string format in `C#` and about the `MVVM`-pattern.
...
In a school there are students and teachers. They have in common that they are persons having a first name, a last name, an age and a sex. Sex is an `enum` which we have to define at first, so add a new file `Sex.cs` into your `Models` folder:
...
WARNING: The order of DataTemplates can be important. You need to order them from most-specific to least-specific (e.g. `Poodle -> Dog -> Animal` not `Animal -> Dog -> Poodle`). https://docs.avaloniaui.net/docs/templates/data-templates#evaluation-order[[more in the docs\]] 
...
If you don't want to use `Resources` for the reused parts, you can also create a https://docs.avaloniaui.net/docs/reference/controls/usercontrol[[`UserControl`\]] for it.  
...
[source,xml]
----
<!-- remember to add the necessary namespace view -->
