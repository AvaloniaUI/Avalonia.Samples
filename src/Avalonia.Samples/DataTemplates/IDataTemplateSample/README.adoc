...
http://reference.avaloniaui.net/api/Avalonia.Controls.Templates/IDataTemplate/[[`IDataTemplate`\]] is an interface which you can implement into your own class to be used as a `DataTemplate`. It has the following members:
...
NOTE: Even if we use an `enum` in this sample, it can be any object in your implementation.
