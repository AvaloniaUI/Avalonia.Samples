...
The model is built up similar to the Basic DataTemplate Sample. We have one class called `Person` and one `enum` called `Sex`:
...
NOTE: We make this class static as nothing depends on instance members. If you need to access instance members, don't make it static. 
...
NOTE: The `Path.Data` is not shown in the above sample code. Please refer to the source code for the necessary path data. 
