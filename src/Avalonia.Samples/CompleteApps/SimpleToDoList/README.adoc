...
At this point, `ToDoItemViewModel` is not yet connected to the `ToDoItem` model. To create a new `ToDoItemViewModel` from an existing `ToDoItem`, we can add a constructor that takes the `ToDoItem` as an argument.
