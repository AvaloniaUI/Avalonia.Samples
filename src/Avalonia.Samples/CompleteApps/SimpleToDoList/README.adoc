...
By default, the Avalonia-MVVM template installs `ReactiveUI.Avalonia` as the MVVM framework. However, we want to use another MVVM framework. Therefore we will uninstall that package and instead install the `CommunityToolkit.Mvvm` package. We need to edit two files to get the changes applied:
