...
In `MainViewModel.cs`, add an `ObservableCollection` called `ToDoItems`. This collection will notify the UI whenever an item is added or removed. Only a getter is needed, as the property can be read-only.

You also need to add two `using` statements to be able to use the `ObservableCollection`.
...
In `MainViewModel.cs`, add a new observable property and give it a name such as `NewItemContent`. Give it a command called `AddItemCommand`, which adds the item if it is a non-empty string. The command and property will be created by the source generator provided by `CommunityToolkit.MVVM` so long as you have used `partial`.
...
NOTE: *Note:* According to the app draft, we want to add the Delete button next to each item. Therefore, we don't need to set `CanExecute` in this case, as we can be certain that the button only ever appears when there is a valid `CommandParameter`.
...
At this point, your IDE may be reporting errors for some of the elements in `MainViewModel.cs`. These will be resolved in the following steps—there is no need to debug extensively right now.
