...
├── AdvancedTodoList.csproj            # Main application project
├── AdvancedTodoList.Android.csproj    # Android platform project
├── AdvancedTodoList.Browser.csproj    # Browser (WebAssembly) platform project
├── AdvancedTodoList.Desktop.csproj    # Desktop platform (Windows/Linux/macOS) project
└── AdvancedTodoList.iOS.csproj        # iOS platform project
----
...
TIP: If you later want to add another platform, it is often easier to create a new x-plat project and copy the necessary files instead of creating those by hand. 
...
In the main application project (typically `App.xaml.cs` or `Program.cs`) we provide a helper method to register all services by providing the `ServiceCollection`. Missing services will be added via the default implementation. That way we only have to provide platform-specific services where absolutely needed.
...
* Register services *before* the app starts—typically in the static constructor or `Main` method
* For platform-specific implementations (like Browser vs. Desktop), use the platform project to register those services.
* For the previewer, you may need to register a special implementation depending on the use-case.
====
...
. No visible windows - tests run in the console
...
* SQLite stored in the app's documents directory
...
video::_docs/final_result-2026-02-16_20.12.27.mp4[title="Final result"]
