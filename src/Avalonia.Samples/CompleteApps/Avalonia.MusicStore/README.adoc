...
In the opened .csproj file, ensure you have the correct CommunityToolkit.Mvvm package version no older than 8.4.2 and 
Avalonia version no older than 11.3.0 or 12.0.0.
...
<UserControl ... >
  <DockPanel>
    <StackPanel DockPanel.Dock="Top">
      <TextBox PlaceholderText="Search for Albums...." />
...
NOTE: You can learn more about `[ObservableProperty]` and `INotifyPropertyChanged` https://docs.avaloniaui.net/docs/data-binding/inotifypropertychanged[[here\]].
...
<UserControl ...
    ...
    <DockPanel>
      <StackPanel DockPanel.Dock="Top">
          <TextBox PlaceholderText="Search for Albums...." Text="{Binding SearchText}" />
...
To implement a real album search in the app, you will use the _Apple iTunes_ Web API album search.
...
==== Apple iTunes Web API

This sample calls the iTunes Search API directly using `HttpClient` and `System.Text.Json` — no additional NuGet package is needed. See https://performance-partners.apple.com/search-api[[the official iTunes Search API documentation\]] for more information about the API.

==== MVVM Model

In this tutorial the application follows a clear separation between the *data model* and the *business service*:

* `Album` (in `/Models`) is a plain data record that holds `Artist`, `Title`, and `CoverUrl`.
* `AlbumService` (in `/Services`) owns all iTunes API calls, HTTP networking, and file-system caching.

This keeps the data model clean and testable, while the service layer handles all external I/O.

Follow this procedure to add the album data model:
...
See https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/record[[microsoft docs\]] for more information about records in C#.

Now create the album service:

- In the solution explorer, right-click the project and click **Add** -> **New Folder**. Name it **Services**.
- Right-click the new **/Services** folder and click **Add** -> **Class**.
- When prompted for the name, type 'AlbumService'.
- Add the following code:
+
[source,csharp]
----
...
[source,xml]
----
<TextBox PlaceholderText="Search for Albums...." Text="{Binding SearchText, Delay=400}" />
----
+
...
** Calls the album service's `SearchAsync` method to fetch data from the iTunes API.
...
Your first step is to extend the `AlbumService` to retrieve the album cover art from the _Apple iTunes_ Web API.

Follow this procedure to add cover loading to `AlbumService`:

- Stop the app if it is still running.
- Locate and open the **AlbumService.cs** file in the **/Services** folder.
- Add the `LoadCoverBitmapAsync` and `SanitizeFileName` methods as shown:
...
Method `LoadCoverBitmapAsync(album)` returns a stream that can be used to load a bitmap from, either from a cache file or from the API.
...
Finally. you will add some code to the album service to save the user's album collection to disk, so that it can be recovered when the app next runs.
...
Follow this procedure to add persistence methods (save and load) to the album service:
...
Your next step is to add a method to the album view model that calls the album service persistence methods:

SaveAsync(album):: persists the album text data as a JSON file,

SaveCoverBitmapStream(album):: saves the cover art as a bitmap (.BMP) file.
...
You have already added code to the album service that can load both the files you will need from disk. All that remains for you to do, is to add some code to the main window view model to handle the start-up.
