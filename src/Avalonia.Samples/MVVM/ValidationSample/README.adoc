...
1. `using` statements for `System.ComponentModel.DataAnnotations` and `CommunityToolkit.Mvvm.ComponentModel`.
2. The `ValidationUsingDataAnnotationViewModel` class as an `ObservableValidator`.
3. The `EMail` observable property.
