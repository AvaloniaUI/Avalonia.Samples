...
- `using` statements for `ReactiveUI`, `System.ComponentModel.DataAnnotations`, and `DataValidationGuide.ViewModels`.
...
*Tip:* The `protected` keyword makes the properties or methods available in derived classes, but not from any external class.
...
In the constructor of our `ViewModel` we will listen to changes of the property `EMail` and run the validation with every change. Moreover, we will run the validation inside the constructor to make sure the initial value is already validated.
