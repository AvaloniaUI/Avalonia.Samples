...
In the first approach we will show you how to use https://learn.microsoft.com/en-us/dotnet/api/system.componentmodel.dataannotations.validationattribute[[`DataAnnotations`\]] to validate a property. To get started, please create a new project using the `Avalonia MVVM Template` and the MVVM Community Toolkit.

=== Step 1: Enable data annotations validation

Beginning with Avalonia 12, the `DataAnnotations` validation is disabled by default. To use this option, you have to https://docs.avaloniaui.net/docs/guides/development-guides/data-validation#manage-validationplugins[[enable the DataAnnotationsValidator\]] in your app.
...
Create a new file called `ValidationUsingDataAnnotationViewModel.cs`. Put this new file in the `ViewModels` folder.
...
In `ValidationUsingDataAnnotationViewModel.cs`, add the following:

- `using` statements for `System.ComponentModel.DataAnnotations` and `CommunityToolkit.Mvvm.ComponentModel`.
- The `ValidationUsingDataAnnotationViewModel` class as an `ObservableValidator`.
- The `EMail` observable property.
...
namespace DataValidationGuide.ViewModels;
...
In `MainWindow.axaml`, add a `TabControl`. Set the `DataContext` of the tab control to `ValidationUsingDataAnnotationViewModel`, and set a corresponding `x:DataType` to allow the binding data type to be inferred.

Inside the tab control, add a `TabItem` containing a single `TextBox` which binds to our property `EMail`. 

.*MainWindow.axaml*
[source,xml]
----
<TabControl TabStripPlacement="Bottom"
            x:DataType="viewModels:ValidationUsingDataAnnotationViewModel">
    <TabControl.DataContext>
        <vm:ValidationUsingDataAnnotationViewModel />
    </TabControl.DataContext>
...
image::_docs/option1.png["Result of option 1"]
...
protected void AddError(string propertyName, string errorMessage):: This void will add a given error message for a given property by its name. The `ErrorsChanged`-event will be raised.
|===
