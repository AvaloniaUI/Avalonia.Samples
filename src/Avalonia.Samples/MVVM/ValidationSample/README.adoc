...
- `using` statements for `ReactiveUI`, `System.ComponentModel.DataAnnotations`, and the `ViewModels` of your project.
...
[TIP]
====
Depending on your IDE-Settings the debugger may stop execution when the `Exception` is hit. You can safely click `Resume` or `Mute and Resume` since Avalonia will handle the exception.
====
