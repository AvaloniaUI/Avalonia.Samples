...
CommunityToolkit-MVVM:: This library ships https://learn.microsoft.com/en-us/dotnet/communitytoolkit/mvvm/relaycommand[[`RelayCommand`\]] and https://learn.microsoft.com/en-us/dotnet/communitytoolkit/mvvm/asyncrelaycommand[[`AsyncRelayCommand`\]]. In addition it provides a https://learn.microsoft.com/en-us/dotnet/communitytoolkit/mvvm/generators/relaycommand[[`source generator`\]] to reduce the biolerplate code needed.  
...
Add a new class into the `ViewModels`-folder called `ReactiveUiCommandsViewModel`. This class should inherit `ViewModelBase` or `ReativeObject`. 
...
In this sample we will show you how to reduce the needed boilderplate code by using the source generators provided. 
...
WARNING: Since our above model was using `ReativeUI`, our `ViewModelBase` inherits `ReactiveObject`. Therefore, we either need to change this or inhierit `ObservableObject` directly.

NOTE: We want to use source generators here. To make this work, or class must be marked as `parial`
...
.CommunityToolkitCommandsViewModel
[source,csharp]
----
/// <summary>
/// The name of a robot. If the name is null or empty, there is no other robot present.
