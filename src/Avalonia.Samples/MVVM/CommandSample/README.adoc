...
ReactiveUI:: This library ships https://www.reactiveui.net/documentation/handbook/commands/[[`ReactiveCommand`\]].
...
Some `MVVM`-libs, provide an async implementation of the `ICommand` interface. That way we can execute long-running tasks from our UI without blocking the UI.
...
----
    public static AppBuilder BuildAvaloniaApp()
        => AppBuilder.Configure<App>()
            .UsePlatformDetect()
            .LogToTrace()
+           .UseReactiveUI(builder => { /* configure this if needed */ });
----
====
...
TIP: When you execute this command, notice that the `Button` is disabled until the `Task` is finished, so you can't ask for the pod bay doors to open again while the opening sequence is running. A more complete program could keep track of the Pod Bay's status and change the button to "close".
...
<StackPanel Orientation="Horizontal" Spacing="5">    
    <TextBox Text="{Binding RobotName}" PlaceholderText="Robot Name" />
...
- https://prismlibrary.com/docs/commands/commanding[[DelegateCommand\]] from `Prism Library`
