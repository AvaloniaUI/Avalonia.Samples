...
This example will show you how to use `Interactions` to show https://docs.avaloniaui.net/docs/basics/user-interface/file-dialogs[[dialogs\]] in your MVVM application
...
We assume you already know how the MVVM pattern works and how dialogs, such as file dialogs, can be shown in general. You should also know what a https://docs.avaloniaui.net/docs/concepts/toplevel[[TopLevel\]]-control in Avalonia is and what it can be used for. 
...
In this section you will learn how to use Reactive UI https://www.reactiveui.net/documentation/handbook/interactions/[[`Interactions`\]] to interact with your `View`.
...
We add a `ReactiveUserControl` as your view. This control allows us to register a handler for our interactions. Let's add this handler in the UserControls code behind:
...
In the constructor of our `ReactiveUserControl` we need to set up the interaction inside `this.WhenActivated` like shown below: 
...
Now we can set up the `XAML` as following. Mind the binding of the `Button` to our `Command` in the `ViewModel`:
...
IDisposable RegisterHandler(Func<TInput, Task<TOutput>> handler):: The View will use this method to register the action to be performed.
...
And this is what the final class looks like:

[source,c#]
----
/// <summary>
/// Simple implementation of Interaction pattern from ReactiveUI framework.
/// https://www.reactiveui.net/documentation/handbook/interactions/
/// </summary>
...
In the next step we add a Command which will call the interaction:
...
Somehow we need to register the `View` to the `Interaction` of the `ViewModel`. In Avalonia we have an event called `OnDataContextChanged` which we can subscribe, or, if we are in code behind, simply override it.
...
WARNING: Remember that the DataContext can change several times. To not get any memory leak, we have to dispose any earlier registration to an older view model
...
* Use a messenger service like the https://learn.microsoft.com/en-us/dotnet/communitytoolkit/mvvm/messenger[CommunityToolkit.Mvvm.Messenger] (see this sample: link:../../CompleteApps/Avalonia.MusicStore/[MusicStore-sample])
