...
TIP: Technically we can also map to `object` or any other base class, but since we later on want to use extension methods, these will leak into any object. Having the interface introduced will only show the extension methods where they are useful. 

Coming back to the DialogManager-class, let's add a static `Dictionary` where we can store the registered mappings between the context (for example: our `ViewModel`) and the `View` or `Control`, which we want to interact with.
...
In the next step we add an https://docs.avaloniaui.net/docs/concepts/attached-property[[AttachedProperty]] to our dialog manager, which we call `Register`. Later we can bind any `IDialogParticipant` to this property from every available `Visual` in our `Views`.
...
NOTE: Because `RegistrationMapper` is a static dictionary, forgetting to remove entries would prevent both the ViewModel and the Visual from being garbage collected. The `DetachedFromVisualTree` handler above ensures entries are cleaned up automatically when, for example, a dialog window is closed.
...
If we are even more lazy, we can add some extension methods that we can use later to call a dialog in one line. To do this, we add a new static class `DialogHelper` into our `Services`-folder. 
...
TIP: Please have a look in the sample code for some more possible options. The `DialogHelper` can be part of a separate controls library or part of your application. A mix is also possible.
