...
Whenever we want to create a new control, we need to derive from a valid base class, which implements the necessary interfaces and functions like styling, layout and user interaction. Below you can find a list of possible base-classes:
...
For more information about the types of controls, please visit the https://docs.avaloniaui.net/docs/custom-controls/[[Documentation\]].
...
AttachedProperty:: An `AttachedProperty` is a property that can be set on any `Control`, even it doesn't define the property on its own (e.g.: `Grid.Row`, `DockPanel.Dock`, …)
...
TemplateParts::
Some `Controls` require specific `Controls` inside the the `ControlTemplate`, which also needs to have a defined name to reference them inside the code behind. By convention these controls have a name with the prefix `"PART_"`. For example, you can use this to listen to events of these template parts.
...
First, we will write a list of requirements: 
...
- There must be visual feedback showing the current rating
...
NOTE: Items listed above containing the word _should_ mean they are nice to have, but the control will also work without this requirement fulfilled. On the other hand, requirements with the word *must* cannot be omitted, as the function of the control will not be given.

Now that we know the functions we want to serve, we can create a simple sketch of what the control should look like: 
...
If we want to create a flexible control, we should not be hardcoding the number of stars. Instead, the developer should be able to define it inside a `Style`. Therefore, we add a `StyledProperty` called `NumberOfStars`. The type of our property is `Integer`, the default value is `5`: 
...
TIP: In this sample the value is of type `int`, so only full stars can be shown. If you want to add support for partial stars, consider to use `float` or `double`.
...
While we can already add a `RatingControl` to our View, we will see nothing as there is no `RatingControl` available. To change this, we add another folder called `Themes`. Add a file called `RatingControlTheme.axaml` which is of type `ResourceDictionary (Avalonia)`. 

First we need to add the necessary namespaces to our `ResourceDictionary`: 
...
[TIP]
====
If you want to have preview of the `ResourceDictionary`, add one or more `RatingControls` to the `Design.PreviewWith`-section: 
[source,xml]
----
<Design.PreviewWith>
...
Last but not least, we want visual feedback if the user hovers a star with their mouse device. So we add a `Style` with the class name `star` and the https://docs.avaloniaui.net/docs/styling/styles#pseudoclasses[[pseudoclass\]] `:pointerover`.
...
NOTE: If you don't want to use `ControlThemes`, you can also use `Styles` to define the look of your control. However, `ControlThemes` have some advantages over `Styles`. Visit the https://docs.avaloniaui.net/docs/styling/control-themes[[Documentation\]] for more information about the differences and advantages of `ControlThemes` over `Styles`.

.Reference
****
* https://docs.avaloniaui.net/docs/styling/styles[Styles]
* https://docs.avaloniaui.net/docs/styling/control-themes[Control Themes]
****
...
----
    ...
    <Style Selector="Path.star:pointerover" >
        <Setter Property="RenderTransform" Value="scale(1.3)" />
        <Setter Property="Stroke" Value="{DynamicResource RatingControlPointerOverStrokeBrush}" />
    </Style>
----

now, we will define Resources for each Theme Variant
...
----
  ...
  <ResourceDictionary.ThemeDictionaries>
    <ResourceDictionary x:Key="Default">
      ...
      <SolidColorBrush x:Key="RatingControlSelectedStrokeBrush" Color="Goldenrod"/>
      <!-- Unselected Brushes-->
      <SolidColorBrush x:Key="RatingControlUnselectedBrush" Color="White"/>
      <SolidColorBrush x:Key="RatingControlUnselectedStrokeBrush" Color="Gray"/>
      <!-- PointerOverBrush -->
      <SolidColorBrush x:Key="RatingControlPointerOverStrokeBrush" Color="Black" />
    </ResourceDictionary>
    <ResourceDictionary x:Key="Light">
      <!-- Selected Brushes-->
      <SolidColorBrush x:Key="RatingControlSelectedBrush" Color="Gold"/>
      <SolidColorBrush x:Key="RatingControlSelectedStrokeBrush" Color="Gray"/>
      <!-- Unselected Brushes-->
      <SolidColorBrush x:Key="RatingControlUnselectedBrush" Color="White"/>
      <SolidColorBrush x:Key="RatingControlUnselectedStrokeBrush" Color="Gray"/>
      <!-- PointerOverBrush -->
    ...
    <ResourceDictionary x:Key="Dark">
      ...
      <SolidColorBrush x:Key="RatingControlSelectedStrokeBrush" Color="White"/>
      <!-- Unselected Brushes-->
      <SolidColorBrush x:Key="RatingControlUnselectedBrush" Color="Transparent"/>
      <SolidColorBrush x:Key="RatingControlUnselectedStrokeBrush" Color="Gray"/>
      <!-- PointerOverBrush -->
      <SolidColorBrush x:Key="RatingControlPointerOverStrokeBrush" Color="White" />
    </ResourceDictionary>
  </ResourceDictionary.ThemeDictionaries>
----
...
[TIP]
====
If you want to have preview of the `ResourceDictionary` support Theme Variants, add one or more `RatingControls` to the `Design.PreviewWith`-section: 
[source,xml]
----
  <!-- Design time preview -->
  <Design.PreviewWith>
  ...
  <Application.Resources>
    <ResourceDictionary>
      <ResourceDictionary.MergedDictionaries>
        ...
        <ResourceInclude Source="/Styles/RatingControlTheme.axaml"/>
      </ResourceDictionary.MergedDictionaries>
    </ResourceDictionary>
  </Application.Resources>
----
