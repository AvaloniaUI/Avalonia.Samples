...
Now we will create a https://docs.avaloniaui.net/docs/controls/usercontrol[[`UserControl`\]] for each page. 
...
Now it's time to set up the file `Views ► MainWindow.axaml`. We will add `Grid` containing two `Buttons` as well as a https://docs.avaloniaui.net/docs/controls/transitioningcontentcontrol[[`TransitioningContentControl`\]]. 
