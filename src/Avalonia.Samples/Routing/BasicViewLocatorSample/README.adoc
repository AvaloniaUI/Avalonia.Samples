...
We can add a class called `ViewLocator` which can be used to create a new instance of a view for the given data object. In this sample we are using reflection to get the necessary views. You can also use different approaches such as a `switch`-statement or a source generator.

TIP: If you create the project using the IDE of your choice, you can opt to create a `ViewLocator` class. If you didn't do it, just create a new class called `ViewLocator` in the root of your project and add the following code to it:
...
This function uses https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/concepts/reflection[[`Reflection`\]] to create a new instance of the `View` representing the given `ViewModel`. First, the full name of the ViewModel is extracted, for example:
+
...
+
In the next step the `ViewLocator` tries to get a `Type` of an object with the given name. If it finds such a `Type`, it will create and return a new instance of the given `Type`. If it didn't find a matching `Type`, it returns a `TextBlock` with a not found message. 

NOTE: By convention the `ViewModel` must be within the `ViewModels`-folder and have "ViewModel" inside its name. The `View` on the other hand must be in the `Views`-folder and have `View` in its name. If you don't like this convention, feel free to change it to your needs. 

TIP: You can also use a `switch`-statement, a `Dictionary` or a source generator instead of reflection. Reflection is not the most performant way to get the correct view, but it is the most flexible one as you don't need to change your `ViewLocator` if you add new pages.
...
Sometimes your `ViewModels` and `Views` may be stored in a shared library. If that is the case, you can add several `ViewLocator` classes. Remember to initiate each one in `App.DataTemplates`.
...
NOTE: the `protected`-modifier lets us implement a setter that is not publicly accessible, but can be overridden in derived classes. https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/protected[[Microsoft Docs\]]

=== Step 3: Create your PageViewModels

Let's create a `ViewModel` for each Wizard page we need. Each `PageViewModel` must implement the earlier introduced <<base-class,base-class>>. 
...
This page will have two input fields called `MailAddress` and `Password`. Inside the constructor of this class we will listen to changes of these properties and set `CanNavigateNext` to `true` if both properties match the requirements. 
...
TIP: We use `DataAnnotations` to validate the user input inside the UI. This is totally optional. You can read more about it in the https://learn.microsoft.com/en-us/dotnet/api/system.componentmodel.dataannotations.validationattribute[[Microsoft Docs\]].

[NOTE] 
====
Using Avalonia 12.0 or higher, you need to opt in for `DataAnnotations` by adding the following line to your `Program.cs`:
...
Now we will create a https://docs.avaloniaui.net/controls/primitives/usercontrol[[`UserControl`\]] for each page. 
...
Now we need to create the navigation logic inside the file `ViewModels ► MainWindowViewModel.cs`. We will add four things: 
...
As you will see in the constructor we will use `WhenAnyValue` to activate or deactivate the `Commands`, depending on if the `CurrentPage` can navigate in the considered direction. 
...
=== Step 6: Set up the MainWindow

Now it's time to set up the file `Views ► MainWindow.axaml`. We will add `Grid` containing two `Buttons` as well as a https://docs.avaloniaui.net/docs/reference/controls/transitioningcontentcontrol[[`TransitioningContentControl`\]]. 
...
You can also use any third-party library that supports advanced routing, for example:

ReactiveUI::
- https://www.reactiveui.net/documentation/handbook/routing/

Prism::
- https://prismlibrary.com/docs/viewmodel-locator
- https://www.reactiveui.net/documentation/handbook/routing/